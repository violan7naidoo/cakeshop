      ...
      {images.map((image, index) => (
        <div key={index} className="relative aspect-square overflow-hidden rounded-lg group">
          ...
          <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center p-4 text-center">
            <h3 className="text-white text-xl font-playfair mb-2">{image.alt}</h3>
            <p className="text-[#E8B4B8] font-semibold mb-1">From {image.startingPrice}</p>
            <div className="text-white/90 text-sm space-y-1">
              <p>
                <span className="text-[#E8B4B8]">Sizes:</span> {image.sizes.join(", ")}
              </p>
              <p>
                <span className="text-[#E8B4B8]">Flavors:</span> {image.flavors.join(", ")}
              </p>
              <p>
                <span className="text-[#E8B4B8]">Decor:</span> {image.decor}
              </p>
            </div>
          </div>
        </div>
